...
# get username and password from os environment variables, or hard code below
...
guest_CLP = os.environ.get("guest_CLP")

urllib3.disable_warnings()

a = Canopy.Acano(CMS_IP, CMS_USER, CMS_PWD)

#a = canopy.Acano(CMS_IP, CMS_USER, CMS_PWD)
auth = "Basic " + base64.b64encode(str.encode(CMS_USER + ":" + CMS_PWD)).decode("utf-8")
...
class CMS:
    ...
    def createSpace(name, PIN, tenantId):
        payload = {
            'name' : name,
            'uri' : name + '.space',
            'requireCallId' : True
